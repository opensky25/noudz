...
class FaselHD : MainAPI() {
    ...
    private  val alternativeUrls = listOf(
        ...
        "https://web3718x.faselhdx.bid",
        "https://asd.pics/home3",
        "https://krmzi.org",
        "https://wecima.date",
        "https://shoffree.cv",
        "https://arablionztv.ink",
        "https://egydead.motorcycles",
        "https://s.3sk.media",
        "https://c4u.top"
    )
    ...
    private fun getAlternativeUrl(): String = alternativeUrls.randomOrNull() ?: alternativeUrls.first()
    ...
    private fun Element.toSearchResponse(): SearchResponse? {
        ...
        return MovieSearchResponse(
            ...
            posterHeaders = cfKiller.getCookieHeaders(getAlternativeUrl()).toMap()
        )
    }
    ...
    override suspend fun getMainPage(page: Int, request : MainPageRequest): HomePageResponse {
        ...
        if(doc.select("title").text() == "Just a moment...") {
            doc = app.get(request.data.replace(mainUrl, getAlternativeUrl()) + page, interceptor = cfKiller, timeout = 120).document
        }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        if(d.select("title").text() == "Just a moment...") {
            d = app.get("${getAlternativeUrl()}/?s=$q", interceptor = cfKiller, timeout = 120).document
        }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        return if (isMovie) {
            newMovieLoadResponse(
                ...
                this.posterHeaders = cfKiller.getCookieHeaders(getAlternativeUrl()).toMap()
            }
        } else {
            ...
            doc.select("div[id=\"seasonList\"] div[class=\"col-xl-2 col-lg-3 col-md-6\"] div.seasonDiv")
                .not(".active").apmap { it ->
                    ...
                    if(s.select("title").text() == "Just a moment...") {
                        s = app.get("${getAlternativeUrl()}/?p="+id, interceptor = cfKiller).document
                    }
